...
def update_pedf(y_substrate, density_cap, density_ecm, ec_old, pedf, pedf_old, k, h, x_length):
    ...
    # Create new PEDF array to help calculate the ECM
    ...
    # Update the PEDF interior point- direct calculation to decrease run time
    for y in range(y_substrate-3, 2, -1):
        if y % 2 == 0:
            for x in range(1, x_steps-2):
                density = density_ecm * (ec_old[y//2][x] + ec_old[y//2][x+1]) / 2
                p[y][x] = 1 / (h * h + 2 * K39 * k) \
                          * (K39 * k * (pedf[y][x+1] + pedf[y][x-1] + pedf[y+2][x] + pedf[y-2][x])
    ...
    for x in range(1, x_steps-2):
        p[2][x] = 1 / (K40 + h) * (K40 * p[4][x] + h * pedf[0][x])
    p[2][0] = p[2][1]
    p[2][x_steps-2] = p[2][x_steps-3]
    for x in range(1, x_steps-1):
        p[1][x] = 1 / (K40 + h) * (K40 * p[3][x] + h * (pedf[0][x] + pedf[0][x-1]) / 2)
    p[1][0] = p[1][1]
