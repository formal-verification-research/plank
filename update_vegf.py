...
def update_vegf(y_substrate, density_cap, density_ecm, ec_old, vegf, vegf_old, k, h, x_length):
    ...
    while tol == 0:
        ...
        for x in range(x_steps-1):
            v[y_substrate-1][x] = K35 * h \
                                    * ((1 - cos(2 * pi * x_coordinate(x, y_substrate - 1, x_steps, x_length))) ** M0) \
                                    + (vegf[y_substrate-2][x] + vegf[y_substrate-2][x+1]) / 2
            ...
            if y % 2 == 0:
                    ...
                    v[y][x] = RELAX1 \
                              ...
                              * ((K21 / 2)
                                 * (vegf[y][x+1] + vegf[y][x-1]
                                    + (vegf[y+1][x] + vegf[y+1][x+1]) / 2 + (vegf[y-1][x] + vegf[y-1][x+1]) / 2
                                    + vegf_old[y][x+1] + vegf_old[y][x-1]
                                    + (vegf_old[y+1][x] + vegf_old[y+1][x+1]) / 2
                                    + (vegf_old[y-1][x] + vegf_old[y-1][x+1]) / 2)
                ...
                for x in range(1, x_steps-1):
                    ...
                    v[y][x] = RELAX1 \
                              ...
                              * ((K21 / 2)
                                 * (vegf[y][x+1] + vegf[y][x-1]
                                    + (vegf[y+1][x-1] + vegf[y+1][x]) / 2 + (vegf[y-1][x-1] + vegf[y-1][x]) / 2
                                    + vegf_old[y][x+1] + vegf_old[y][x-1]
                                    + (vegf_old[y+1][x-1] + vegf_old[y+1][x]) / 2
                                    + (vegf_old[y-1][x-1] + vegf_old[y-1][x]) / 2)
                                 + (h * h / k - 2 * K21 - h * h * K1 * density / (1 + vegf_old[y][x]))
                                 * vegf_old[y][x]) \
                              + (1 - RELAX1) * vegf[y][x]
        ...
        for x in range(1, x_steps-1):
            v[1][x] = 1 / (K33 + h) * (K33 * (vegf[2][x] + vegf[2][x-1]) / 2 + h * (vegf[0][x] + vegf[0][x-1]) / 2)
            v[1][0] = v[1][1]
