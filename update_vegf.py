...
def update_vegf(y_substrate, density_cap, density_ecm, ec_old, vegf, vegf_old, k, h, x_length):
    ...
    for x in range(x_steps-1):
        ...
        vegf_cap = (vegf_old[1][x] + vegf_old[1][x+1]) / 2
        vegf[0][x] = vegf_old[0][x] + k * (-K1 * vegf_old[0][x] * density
                                       / (1 + vegf_old[0][x]) + K2 * (vegf_cap - vegf_old[0][x]))
        if vegf[0][x] < 0:
            vegf[0][x] = 0
    ...
    # Update the VEGF in the blood vessel wall and in the ECM
    tol = 0
    while tol == 0:
        tol = 1
        ...
        for x in range(1, x_steps-2):
            v[y_substrate-1][x] = K35 * h \
                                    * ((1 - cos(2 * pi * x_coordinate(x, y_substrate - 1, x_steps, x_length))) ** M0) \
                                    + (v[y_substrate-2][x] + v[y_substrate-2][x+1]) / 2
        v[y_substrate - 1][0] = v[y_substrate - 1][1]
        v[y_substrate-1][x_steps-2] = v[y_substrate-1][x_steps-3]

        # Update the interior points using Plank Eq 70 Pg 179
            ...
            if y % 2 == 0:
                for x in range(1, x_steps-2):
                    ...
                    v[y][x] = RELAX1 \
                              * (k / (h * h + 2 * K21 * k)) \
                              * (K21 / 2) \
                              * (v[y][x+1] + v[y][x-1] + v[y+1][x] + v[y+1][x+1] + v[y-2][x]
    ...
    # # Create two VEGF substrate arrays to use to iterate to a new solution for each node
    # v = zeros((y_substrate, x_steps))
