...
# Function
def pMove(x, y, direction, protease, fibronectin, vegf, xSteps, ySteps, lam, k):
    ...
    # Left
    if x > 0:
        T[0] = tau(protease[2*y][x-1], fibronectin[2*y][x-1], vegf[2*y][x-1], y)
    else:
        T[0] = 0
    ...
    if x < xSteps-1:
        T[1] = tau(protease[2*y][x], fibronectin[2*y][x], vegf[2*y][x], y)
    else:
        T[1] = 0

    # Up
    if y > 1:
        T[2] = tau(protease[2*y-1][x], fibronectin[2*y-1][x], vegf[2*y-1][x], y)
    else:
        T[2] = 0

    # Down
    if 0 < y < ySteps - 1:
        T[3] = tau(protease[2*y+1][x], fibronectin[2*y+1][x], vegf[2*y+1][x], y)
    else:
        T[3] = 0
