...
def startup():
    ...
    y_steps = x_steps / 2  # Since the depth is 1/2 of L it also has 1/2 the steps
    ...
    vegf = zeros((y_steps, x_steps))  # VEGF array
    pedf = zeros((y_steps, x_steps))  # PEDF array
    pro = zeros((y_steps, x_steps))  # Pro array
    fib = ones((y_steps, x_steps))  # Fib array
    vegf_old = zeros((y_steps, x_steps))  # Old VEGF array
    pedf_old = zeros((y_steps, x_steps))  # Old PEDF array
    pro_old = zeros((y_steps, x_steps))  # Old Pro array
    fib_old = ones((y_steps, x_steps))  # Old Fib array
    ...
    return start_time, x_length, y_length, y_steps, file_events, total_time, total_number_time_steps, k, h, lam, \
           x_vector, y_vector, x_position, y_position, death_time, birth_time, divide_time, vegf, pedf, pro, fib, \
           vegf_old, pedf_old, pro_old, fib_old, model, ec, ec_old, density_cap, density_ecm, cell_lineage, \
